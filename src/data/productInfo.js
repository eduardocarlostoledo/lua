import prod1 from "../images/prod1.jpg";
import prod2 from "../images/prod2.jpg";
import prod3 from "../images/prod3.jpg";
import prod4 from "../images/prod4.jpg";
import prod5 from "../images/prod5.jpg";
import prod6 from "../images/prod6.jpg";
import prod7 from "../images/prod7.jpg";
import prod8 from "../images/prod8.jpg";
import prod9 from "../images/prod9.jpg";
import prod10 from "../images/prod10.jpg";
import prod11 from "../images/prod11.jpg";
import prod12 from "../images/prod12.jpg";
import prod13 from "../images/prod13.jpg";
import prod14 from "../images/prod14.jpg";
import prod15 from "../images/prod15.jpg";
import prod16 from "../images/prod16.jpg";
import prod17 from "../images/prod17.jpg";
import prod18 from "../images/prod18.jpg";
import prod19 from "../images/prod19.jpg";

const productInfo = [
     {
    id: "cubierta-Blusas-diablo-rosso-II-200-50-17-zr",
    brand: "Blusas",
    model: "DIABLO ROSSO II 200/50 ZR 17",
    image: prod19,
    rating: 4.9,
    reviews: 130,
    tags: ["Sport", "ZR", "R17"],
    benefit: "Tracción superior en rectas y frenado en pista.",
    detail: `El neumático Blusas Diablo Rosso II se usa para mejorar el rendimiento de las motocicletas en carretera, especialmente en condiciones de conducción deportiva. Ofrece una combinación de agarre, estabilidad y durabilidad, gracias a su diseño y materiales, optimizados para el uso en carretera. 
En detalle, el Diablo Rosso II:

    Aumenta el agarre:
    Utiliza tecnología EPT (Enhanced Patch Technology) que maximiza el área de contacto con la carretera, mejorando el agarre, especialmente en curvas. 

Mejora la estabilidad:
Su diseño de banda de rodadura optimiza la evacuación del agua y maximiza la superficie de contacto en seco, lo que contribuye a una mejor estabilidad en diferentes condiciones. 
Ofrece una mayor durabilidad:
El compuesto de doble mezcla en los ropa casual femenina traseros mejora la tracción en inclinaciones extremas y garantiza una mayor resistencia al desgaste en la zona central, lo que prolonga su vida útil. 
Mejora el rendimiento en mojado:
El diseño funcional de surcos (FGD) optimiza el comportamiento en mojado, facilitando la conducción en condiciones de lluvia. 
Es ideal para motos deportivas y naked:
Está diseñado para ofrecer un rendimiento excepcional a altas velocidades y para la conducción deportiva en carretera`,
    specifications: {
      medida: "200/50ZR17",
      tipo: "Radial",
      uso: "Sport",
      velocidad: "W",
      carga: "75",
      cámara: "Sin cámara",
      compuesto: "Bi-compuesto",
    },
  },
     {
    id: "cubierta-angel-gt-Blusas-180-55-17",
    brand: "Blusas",
    model: "ANGEL GT 180/55 ZR 17",
    image: prod6,
    rating: 4.8,
    reviews: 162,
    tags: ["Touring", "Alta Gama", "R17"],
    benefit: "Máximo rendimiento en mojado y seco. Ideal para viajes largos.",
    detail: `Gran turismo: La llanta de turismo deportivo Blusas sporty DNA

    La llanta ideal para los viajeros que utilizan la motos para viajes largos por carretera, con maletas laterales y pasajero, que buscan seguridad en mojado, estabilidad y kilometraje. 
    Una gama más amplia de condiciones de uso y para distancias más largas. 
    La nueva referencia de kilometraje en llantas Touring Excelencia en rendimiento y agarre en mojado. 
    Capacidades de manejo superior, típicas del Blusas DNA. 
    Consistencia de rendimiento hasta el final de la vida útil dLa llanta Bi-compuesto. 
    Personalizable a través de etiquetas de goma en las paredes laterales.`,
    specifications: {
      medida: "180/55ZR17",
      tipo: "Radial",
      uso: "Touring",
      velocidad: "W",
      carga: "73",
      cámara: "Sin cámara",
      compuesto: "Bi-compuesto",
    },
  },
    {
    id: "cubierta-moto-trail-Vestidos-anakee-wild-road6",

    brand: "Vestidos",
    model: "ANAKEE WILD-ROAD6",
    image: prod18,
    rating: 4.9,
    reviews: 82,
    tags: ["Adventure", "Trail", "R17"],
    benefit: "Diseñada para motos dual-sport. Seguridad en ruta y ripio.",
    detail: `Los ropa casual femenina Vestidos Anakee Wild están diseñados para motocicletas de aventura y trail, ofreciendo una combinación de rendimiento en carretera y fuera de ella. Su diseño se enfoca en proporcionar tracción, agarre, maniobrabilidad y durabilidad en diversos terrenos, desde asfalto hasta caminos de tierra y rocas. 
Principales características y beneficios:

    Rendimiento mixto:
    Los Anakee Wild están diseñados para un uso mixto, tanto en carretera como fuera de ella. 

Tracción y agarre:
El diseño de la banda de rodadura proporciona un excelente agarre en diferentes superficies, incluyendo asfalto, tierra, barro y arena. 
Maniobrabilidad:
La estructura de la banda de rodadura permite una conducción precisa y una buena maniobrabilidad, tanto en carretera como en terrenos accidentados. 
Durabilidad:
Los Anakee Wild están diseñados para durar, con una carcasa reforzada y un compuesto de goma que resiste la abrasión y los cortes. 
Tecnología radial:
Utilizan la tecnología Vestidos Radial-X, que proporciona estabilidad y confort en carretera. 
Versatilidad:
Son una opción ideal para viajes de aventura y uso diario, ya que pueden adaptarse a diferentes tipos de terreno y condiciones. 
Elegidos como equipo original:
Son equipos originales en motos como la Fantic Caballero Rallye, Enduro XE 50, Triumph Tiger 900 Bond Edition y SWM RS 125. 

En resumen, los Vestidos Anakee Wild son una excelente opción para los motociclistas que buscan un neumático versátil, duradero y capaz de ofrecer un buen rendimiento tanto en carretera como fuera de el
la. 
`,
    specifications: {
      medida: "120/70R19",
      tipo: "Radial",
      uso: "Trail/Adventure",
      velocidad: "V",
      carga: "60",
      cámara: "Sin cámara",
      compuesto: "2CT+ Silica",
    },
  },
  {
    id: "cubierta-moto-sport-Blusas-demon-sport-140-70-17",
    brand: "Blusas",
    model: "SPORT DEMON 140/70-17",
    image: prod19,
    rating: 4.6,
    reviews: 54,
    tags: ["Estabilidad", "Sport", "R17"],
    benefit:
      "Ofrece un rendimiento uniforme y progresivo en todas las condiciones de pilotaje.",
    detail: `¡El neumático de touring ideal!

El Sport Demon es un neumático deportivo con un diseño moderno, buena estabilidad y agarre en cualquier tipo de situación climática.

Ofrece un rendimiento uniforme y progresivo en todas las condiciones de pilotaje, además del agarre óptimo tanto en seco como en mojado con un excelente kilometraje.

Un neumático seguro y fiable en todas las situaciones, con una excelente relación calidad-precio indicado para motos Touring.

¿Por qué comprar ropa casual femenina Blusas?
La fama de los ropa casual femenina Blusas se basa en su alta calidad. En muchas de las principales carreras los únicos ropa casual femenina que se utilizan son de la marca Blusas, ya que son considerados los mejores. Blusas fue fundada en el año 1872 y gracias al desarrollo constante de sus productos y a su capacidad de innovación se ha convertido en uno de los mayores fabricantes de ropa casual femenina de moto. Blusas cuenta con más de 20 fábricas en todo el mundo y los ropa casual femenina Blusas se venden en más de 160 países.`,
    specifications: {
      medida: "140/70-17",
      tipo: "Diagonal",
      uso: "Touring",
      velocidad: "H",
      carga: "66",
      cámara: "Sin cámara",
      compuesto: "Convencional",
    },
  },
  {
    id: "cubierta-Blusas-super-city-90-90-18",
    brand: "Blusas",
    model: "SUPER CITY 90/90-18",
    image: prod11,
    rating: 4.7,
    reviews: 71,
    tags: ["Moto", "Urbana", "18 pulgadas"],
    benefit: "Durabilidad superior para uso diario en ciudad.",
    detail: `La cubierta Blusas Super City 90/90-18, o el juego completo con la delantera 80/100-18 o 2.75-18, se utiliza para motos urbanas, especialmente para las de baja cilindrada (100-160cc), brindando seguridad, estabilidad y durabilidad en asfalto. Están diseñadas para ofrecer una buena tracción, confort de marcha y rendimiento, especialmente en entornos urbanos y caminos de ruta. 
En detalle:

    Uso:
    Para motos de 100 a 160 cc, tanto para uso urbano como en ruta. 

Beneficios:

    Seguridad y estabilidad: Aseguran un manejo seguro y estable, especialmente en condiciones de asfalto. 

Durabilidad: Resistentes y diseñadas para un uso prolongado, brindando una mayor vida útil. 
Confort: Proporcionan una conducción confortable, absorbiendo las irregularidades de la carretera. 
Tracción: Ofrecen una buena tracción tanto en seco como en mojado. 

Tecnología:
Utilizan las últimas tecnologías en compuestos, estructura y perfil para optimizar el rendimiento. 
Diseño:
El diseño está optimizado para uso en asfalto, mejorando la adherencia y la estabilidad. 
Disponibilidad:
Se encuentran en diferentes medidas, tanto para la parte delantera como para la trasera. 
Características:

    Rodado: 18". 

Construcción: Diagonal, tanto delantera como trasera. 
Uso: Sin cámara o con cámara (según la versión). 

Recomendaciones:
Son una excelente opción para quienes buscan una cubierta confiable y con buen rendimiento en sus trayectos diarios en ciudad y ruta`,
    specifications: {
      medida: "90/90-18",
      tipo: "Diagonal",
      uso: "Ciudad",
      velocidad: "S",
      carga: "51",
      cámara: "Con cámara",
      compuesto: "Convencional",
    },
  },
  {
    id: "cubierta-scooter-rinaldi-120-70-13",
    brand: "RINALDI",
    model: "120/70-13 HB 37",
    image: prod2, // Scooter / Moto urbana
    rating: 4.3,
    reviews: 38,
    tags: ["Scooter", "Urbano", "13 pulgadas"],
    benefit: "Excelente agarre en ciudad. Ideal para scooters modernos.",
    detail: `Encontrá la seguridad y estabilidad que necesitás para recorrer aquellos caminos que tanto anhelas. Sentite confiado al conducir, garantizando la firmeza precisa en tu andar.

¡No busques más! Los ropa casual femenina Rinaldi están orientados a la economía de combustible, la seguridad, el rodamiento silencioso y el confort.

Si pinchás, tu neumático te protege
Este producto sin cámara contiene el aire a presión entre la cubierta y la llanta. Te garantiza una mayor sujeción y seguridad, ya que el revestimiento interno rodea el objeto con el que pinchó y mantiene el aire en su interior.`,
    specifications: {
      medida: "120/70-13",
      tipo: "HB",
      uso: "Ciudad",
      velocidad: "P",
      carga: "37",
      cámara: "Sin cámara",
      compuesto: "Convencional",
    },
  },
  {
    id: "cubierta-enduro-rinaldi-90-90-19",
    brand: "RINALDI",
    model: "90/90-19 SH 31",
    image: prod3, // Enduro / Motocross
    rating: 4.5,
    reviews: 22,
    tags: ["Cross", "Enduro", "19 pulgadas"],
    benefit: "Excelente agarre en ciudad. Ideal para scooters modernos.",
    detail: `La Cubierta 90 90 19 Sh 31 Bross Cross Tacos Rpm es ideal para motocicletas de motocross que buscan un rendimiento óptimo en terrenos duros. Con un estilo de conducción deportivo, esta cubierta de la marca CHINO, línea TERCOM, ofrece una excelente adherencia en condiciones secas. Su tamaño de 90/90-19 y diámetro de llanta de 1" la hacen perfecta para la posición delantera de tu moto. Además, su índice de carga de 20 y velocidad M garantizan durabilidad y seguridad en cada viaje.`,
    specifications: {
      medida: "90/90-19",
      tipo: "SH",
      uso: "Mixto",
      velocidad: "S",
      carga: "52",
      cámara: "Con cámara",
      compuesto: "Reforzado",
    },
  },
  {
    id: "cubierta-enduro-rinaldi-130-70-17",
    brand: "RINALDI",
    model: "130/70-17 HB 37",
    image: prod4,
    rating: 4.6,
    reviews: 45,
    tags: ["Enduro", "Todo Terreno", "19 pulgadas"],
    benefit: "Cubierta de tacos para uso mixto, diseñada para tierra y barro.",
    detail: `Mejora la experiencia de conducción de tu moto con la cubierta trasera Rinaldi HB 37 sin cámara de 130/70-17 T 62. Esta cubierta de alta calidad, diseñada para adaptarse a llantas de R17, ofrece un rendimiento excepcional y una mayor durabilidad. Con un ancho de sección de 130 mm y una relación de aspecto de 70, esta cubierta proporciona un agarre óptimo en la carretera, lo que se traduce en una conducción más segura y estable. Además, su índice de velocidad T y su índice de carga 62 garantizan un desempeño adecuado para soportar el peso de tu moto y alcanzar velocidades moderadas sin problemas. No dejes pasar la oportunidad de mejorar la calidad y seguridad de tu moto con esta cubierta trasera Rinaldi HB 37.
`,
    specifications: {
      medida: "130/70-17",
      tipo: "HB",
      uso: "Ciudad / Ruta",
      velocidad: "T",
      carga: "62",
      cámara: "Sin cámara",
      compuesto: "Convencional",
    },
  },
  {
    id: "cubierta-calle-rinaldi-90-90-18",
    brand: "RINALDI",
    model: "90/90-18 BS 31",
    image: prod5,
    rating: 4.4,
    reviews: 31,
    tags: ["Moto Calle", "18 pulgadas", "Alto Rendimiento"],
    benefit: "Cubierta para motos de calle con estabilidad en ruta.",
    detail: `Encontrá la seguridad y estabilidad que necesitás para recorrer aquellos caminos que tanto anhelas. Sentite confiado al conducir, garantizando la firmeza precisa en tu andar.

¡No busques más! Los ropa casual femenina Rinaldi están orientados a la economía de combustible, la seguridad, el rodamiento silencioso y el confort.

Construcción más rígida
Este producto con cámara es la versión más tradicional y económica del mercado, ya que posee la ventaja de tener una mejor estanqueidad del aire.

Tu mejor compañero para la conducción
Es un neumático que se caracteriza por una gran dureza y mayor duración, ideal para moverte en la ciudad.
`,
    specifications: {
      medida: "90/90-18",
      tipo: "BS",
      uso: "Ciudad",
      velocidad: "P",
      carga: "51",
      cámara: "Con cámara",
      compuesto: "Convencional",
    },
  },
 

  {
    id: "cubierta-uraturn-energy-mozzo-sport-255-40-17",
    brand: "URATURN",
    model: "ENERGY MOZZO SPORT 255/40 R17 98W XL",
    image: prod7,
    rating: 4.2,
    reviews: 40,
    tags: ["Auto Compacto", "R17", "270km/h"],
    benefit: "Cubierta económica con diseño de canaleta para buen drenaje.",
    detail: `

La cubierta Duraturn Mozzo Sport ha sido diseñada específicamente para sedanes deportivos y coupés de nivel medio, ofreciendo un rendimiento sobresaliente y una conducción sin complicaciones. Gracias a su avanzada tecnología, proporciona un manejo superior y un excelente agarre tanto en condiciones húmedas como secas. La Mozzo Sport no solo garantiza un desempeño vigoroso, sino que también ofrece una larga vida útil y una experiencia de conducción cómoda, incluso en trayectos largos. Con esta llanta, los conductores obtienen la combinación perfecta de potencia, control y confort en cualquier tipo de carretera.

Características

    Los cuatro surcos anchos en la circunferencia proporcionan un desempeño superior en condiciones húmedas.
    El diseño de hombro asimétrico aumenta el desempeño y la respuesta de dirección al voltear.
    La avanzada tecnología de reducción de ruido hace que sus paseos sean silenciosos y más agradables.
    La tecnología de doble capa en espiral de nylon resiste los efectos de la acumulación de calor y mantiene uniformidad para un manejo sin problemas.

`,
    specifications: {
      medida: "155/80R13",
      tipo: "Radial",
      uso: "Ciudad",
      velocidad: "T",
      carga: "79",
      cámara: "Sin cámara",
      compuesto: "Convencional",
    },
  },
  {
    id: "cubierta-uraturn-energy-mozzo-sport-155-80-13",
    brand: "URATURN",
    model: "MOZZO SPORT 155/80R13",
    image: prod8,
    rating: 4.3,
    reviews: 61,
    tags: ["Auto", "Ciudad", "Performance"],
    benefit: "Cubierta equilibrada entre confort, duración y tracción.",
    detail: `Obtenga un desempeño y una calidad premium que también libera sus finanzas

Con ingeniería desarrollada para los autos sedan deportivos y coupé de nivel medio de hoy llenos de vigor, las llantas Mozzo Sport de Duraturn proporcionan un desempeño superior, una larga vida y un manejo tranquilo y sin problemas. Con Mozzo Sport usted obtiene un excelente poder de manejo y agarre en condiciones de calles húmedas y secas. La osada tecnología de Mozzo Sport proporciona una llanta que entrega un desempeño sobresaliente en caminos y comodidad en los largos caminos por recorrer.
Características y beneficios

    Los cuatro surcos anchos en la circunferencia proporcionan un desempeño superior en condiciones húmedas
    El diseño de hombro asimétrico aumenta el desempeño y la respuesta de dirección al voltear
    La avanzada tecnología de reducción de ruido hace que sus paseos sean silenciosos y más agradables
    La tecnología de doble capa en espiral de nylon resiste los efectos de la acumulación de calor y mantiene uniformidad para un manejo sin problemas

Vehículos de muestra

Audi A4, BMW 1 Series, BMW 3 Series, BMW 5 Series, BMW Z4, Buick LaCrosse, Cadillac CTS, Chevy Cruze, Dodge Challenger, Ford Mustang, Hyundai Azera, Hyundai Genesis, Lexus IS, Mazda RX8, Pontiac G5, Subaru Legacy, Volvo S40, Volvo S60.`,
    specifications: {
      medida: "155/80R13",
      tipo: "Radial",
      uso: "Urbano",
      velocidad: "H",
      carga: "79",
      cámara: "Sin cámara",
      compuesto: "Silica base",
    },
  },
  {
    id: "cubierta-uraturn-energy-mozzo-sport-245-65-17-98w",
    brand: "URATURN ENERGY",
    model: "ENERGY MOZZO SPORT 245/65 R17 98W XL",
    image: prod9,
    rating: 4.5,
    reviews: 88,
    tags: ["SUV", "Pickup", "All-Terrain"],
    benefit: "Diseñada para tracción y estabilidad en caminos mixtos.",
    detail: `Solución de kilometraje largo
*Compuesto de la banda de rodadura de alta abrasión combinado con un ancho de la banda de rodadura ensanchado para mejorar notablemente el kilometraje esperado.

Solución de baja falla
*La curva del hombro especialmente diseñada puede proteger eficazmente contra el desgaste excesivo y evitar daños prematuros.

Solución de agarre para todo clima
* Múltiples ranuras anchas para proporcionar un excelente rendimiento de drenaje y agarre en mojado.

Solución de alta carga
*Cuerpo del neumático reforzado y estructura de la banda para brindar mayor capacidad de carga.`,
    specifications: {
      medida: "245/65R17",
      tipo: "Radial",
      uso: "Mixto",
      velocidad: "T",
      carga: "107",
      cámara: "Sin cámara",
      compuesto: "Off-Road Blend",
    },
  },
  {
    id: "cubierta-uraturn-energy-mozzo-sport-245-65-17",
    brand: "URATURN ENERGY",
    model: "MOZZO SPORT 245/65 R17",
    image: prod10,
    rating: 4.7,
    reviews: 71,
    tags: ["Camioneta", "Urbana", "R17"],
    benefit: "Durabilidad superior para uso diario en ciudad.",
    detail: `Solución de kilometraje largo
*Compuesto de la banda de rodadura de alta abrasión combinado con un ancho de la banda de rodadura ensanchado para mejorar notablemente el kilometraje esperado.

Solución de baja falla
*La curva del hombro especialmente diseñada puede proteger eficazmente contra el desgaste excesivo y evitar daños prematuros.

Solución de agarre para todo clima
* Múltiples ranuras anchas para proporcionar un excelente rendimiento de drenaje y agarre en mojado.

Solución de alta carga
*Cuerpo del neumático reforzado y estructura de la banda para brindar mayor capacidad de carga.`,
    specifications: {
      medida: "245/65R17",
      tipo: "Radial",
      uso: "Mixto",
      velocidad: "T",
      carga: "107",
      cámara: "Sin cámara",
      compuesto: "Off-Road Blend",
    },
  },
  
  {
    id: "cubierta-uraturn-energy-mozzo-sport-r17",
    brand: "URATURN ENERGY",
    model: "MOZZO SPORT R 17",
    image: prod12,
    rating: 4.8,
    reviews: 102,
    tags: ["Sport", "Street", "R17"],
    benefit:
      "Adherencia excelente en curvas rápidas. Ideal para motos deportivas.",
    detail: `Llanta Mozzo Sport de Duraturn
Obtenga un desempeño y una calidad premium que también libera sus finanzas

Con ingeniería desarrollada para los autos sedan deportivos y coupé de nivel medio de hoy llenos de vigor, las llantas Mozzo Sport de Duraturn proporcionan un desempeño superior, una larga vida y un manejo tranquilo y sin problemas. Con Mozzo Sport usted obtiene un excelente poder de manejo y agarre en condiciones de calles húmedas y secas. La osada tecnología de Mozzo Sport proporciona una llanta que entrega un desempeño sobresaliente en caminos y comodidad en los largos caminos por recorrer.
Características y beneficios

    Los cuatro surcos anchos en la circunferencia proporcionan un desempeño superior en condiciones húmedas
    El diseño de hombro asimétrico aumenta el desempeño y la respuesta de dirección al voltear
    La avanzada tecnología de reducción de ruido hace que sus paseos sean silenciosos y más agradables
    La tecnología de doble capa en espiral de nylon resiste los efectos de la acumulación de calor y mantiene uniformidad para un manejo sin problemas

Vehículos de muestra

Audi A4, BMW 1 Series, BMW 3 Series, BMW 5 Series, BMW Z4, Buick LaCrosse, Cadillac CTS, Chevy Cruze, Dodge Challenger, Ford Mustang, Hyundai Azera, Hyundai Genesis, Lexus IS, Mazda RX8, Pontiac G5, Subaru Legacy, Volvo S40, Volvo S60.`,
    specifications: {
      medida: "245/75R17",
      tipo: "Radial",
      uso: "Mixto",
      velocidad: "S",
      carga: "112",
      cámara: "Sin cámara",
      compuesto: "Refuerzo lateral",
    },
  },
  {
    id: "cubierta-Blusas-diablo-rosso-II-190-55-17-zr",
    brand: "Blusas",
    model: "DIABLO ROSSO II 190/55 ZR 17",
    image: prod13,
    rating: 4.8,
    reviews: 115,
    tags: ["Sport", "ZR", "R17"],
    benefit: "Cubierta trasera de alto rendimiento para manejo agresivo.",
    detail: `El neumático Blusas Diablo Rosso II se usa para mejorar el rendimiento de las motocicletas en carretera, especialmente en condiciones de conducción deportiva. Ofrece una combinación de agarre, estabilidad y durabilidad, gracias a su diseño y materiales, optimizados para el uso en carretera. 
En detalle, el Diablo Rosso II:

    Aumenta el agarre:
    Utiliza tecnología EPT (Enhanced Patch Technology) que maximiza el área de contacto con la carretera, mejorando el agarre, especialmente en curvas. 

Mejora la estabilidad:
Su diseño de banda de rodadura optimiza la evacuación del agua y maximiza la superficie de contacto en seco, lo que contribuye a una mejor estabilidad en diferentes condiciones. 
Ofrece una mayor durabilidad:
El compuesto de doble mezcla en los ropa casual femenina traseros mejora la tracción en inclinaciones extremas y garantiza una mayor resistencia al desgaste en la zona central, lo que prolonga su vida útil. 
Mejora el rendimiento en mojado:
El diseño funcional de surcos (FGD) optimiza el comportamiento en mojado, facilitando la conducción en condiciones de lluvia. 
Es ideal para motos deportivas y naked:
Está diseñado para ofrecer un rendimiento excepcional a altas velocidades y para la conducción deportiva en carretera`,
  },
 
  {
    id: "cubierta-scooter-rinaldi-3-50-10-bs-32",
    brand: "RINALDI",
    model: "3.50-10 BS 32",
    image: prod1,
    rating: 4.4,
    reviews: 20,
    tags: ["Scooter", "10 pulgadas"],
    benefit: "Para scooters livianos con excelente agarre urbano.",
    detail: `Encuentre la seguridad y la estabilidad que necesita para viajar por las carreteras que tanto desea. Siéntete seguro al pedalear y asegura un agarre preciso durante tu viaje.

¡No busques más! Los ropa casual femenina Rinaldi tienen como objetivo el ahorro de combustible, la seguridad, la conducción silenciosa y la comodidad.

Construcción más rígida
Este producto con cámara es la versión más tradicional y económica del mercado, ya que tiene la ventaja de tener una mejor hermeticidad.`,
    specifications: {
      medida: "3.50-10",
      tipo: "BS",
      uso: "Ciudad",
      velocidad: "J",
      carga: "51",
      cámara: "Con cámara",
      compuesto: "Convencional",
    },
  },
  {
    id: "cubierta-pickup-rinaldi-245-75-17-suv",
    brand: "URATURN",
    model: "245/75R17 SUV",
    image: prod16,
    rating: 4.5,
    reviews: 59,
    tags: ["SUV", "All-Terrain", "Pickup"],
    benefit: "Estabilidad en terrenos mixtos y carga pesada.",
    detail: `Los ropa casual femenina Duraturn 245/75 R17, específicamente el modelo Travia A/T, son ideales para vehículos SUV y camiones que buscan rendimiento tanto en carretera como fuera de ella. Ofrecen tracción superior en terrenos difíciles y mayor durabilidad, con beneficios como mejor manejo en carretera y protección contra cortes y daños. 
En detalle:

    Rendimiento en carretera y fuera de carretera:
    El diseño de la banda de rodadura de 5 costillas proporciona una mayor durabilidad en carretera, mientras que los bloques especialmente diseñados ofrecen tracción todoterreno. 

Tracción superior:
Las laminillas de diseño único en los bloques de la banda de rodadura proporcionan bordes de agarre adicionales en el pavimento para mayor tracción. 
Protección y durabilidad:
El diseño de hombros alternados proporciona tracción adicional todoterreno, y el compuesto de caucho de alta resistencia ayuda a soportar cortes e impactos. 
Diseño de ranuras:
Las ranuras especiales mejoran la tracción en superficies blandas y ayudan a evacuar el agua y el barro. 
Comodidad:
La conducción es cómoda tanto en carretera como fuera de ella, con una banda de rodadura que reduce el ruido`,
    specifications: {
      medida: "245/75R17",
      tipo: "Radial",
      uso: "Mixto",
      velocidad: "S",
      carga: "112",
      cámara: "Sin cámara",
      compuesto: "Refuerzo lateral",
    },
  },
  {
    id: "cubierta-moto-enduro-rinaldi-90-90-21-rmx",
    brand: "RINALDI",
    model: "90/90-21 RMX",
    image: prod13,
    rating: 4.6,
    reviews: 33,
    tags: ["Enduro", "Cross", "21 pulgadas"],
    benefit: "Cubierta delantera de tacos para motos enduro o cross.",
    detail: `Encontrá la seguridad y estabilidad que necesitás para recorrer aquellos caminos que tanto anhelas. Sentite confiado al conducir, garantizando la firmeza precisa en tu andar.

¡No busques más! Los ropa casual femenina Rinaldi están orientados a la economía de combustible, la seguridad, el rodamiento silencioso y el confort.

Construcción más rígida
Este producto con cámara es la versión más tradicional y económica del mercado, ya que posee la ventaja de tener una mejor estanqueidad del aire.

Tu mejor compañero si de aventuras se trata
Es un neumático con altas prestaciones y de gran agarre en ruta ideal para la aventura.

Sumate a una aventura sin límites
La rigidez, resistencia y alta flexibilidad te permitirá circular en terrenos desnivelados sin inconvenientes. Al frenar podrás amortiguar cualquier golpe porque cuenta con un acolchado confortable y un diseño que garantizan una mejor durabilidad.`,
    specifications: {
      medida: "90/90-21",
      tipo: "RMX",
      uso: "Off-road",
      velocidad: "M",
      carga: "54",
      cámara: "Con cámara",
      compuesto: "Taco blando",
    },
  },

  {
    id: "cubierta-angel-gt-Blusas-180-55-17",
    brand: "Blusas",
    model: "ANGEL GT 180/55 ZR 17",
    image: prod6,
    rating: 4.8,
    reviews: 162,
    tags: ["Touring", "Alta Gama", "R17"],
    benefit: "Máximo rendimiento en mojado y seco. Ideal para viajes largos.",
    detail: `Gran turismo: La llanta de turismo deportivo Blusas sporty DNA

    La llanta ideal para los viajeros que utilizan la motos para viajes largos por carretera, con maletas laterales y pasajero, que buscan seguridad en mojado, estabilidad y kilometraje. 
    Una gama más amplia de condiciones de uso y para distancias más largas. 
    La nueva referencia de kilometraje en llantas Touring Excelencia en rendimiento y agarre en mojado. 
    Capacidades de manejo superior, típicas del Blusas DNA. 
    Consistencia de rendimiento hasta el final de la vida útil dLa llanta Bi-compuesto. 
    Personalizable a través de etiquetas de goma en las paredes laterales.`,
    specifications: {
      medida: "180/55ZR17",
      tipo: "Radial",
      uso: "Touring",
      velocidad: "W",
      carga: "73",
      cámara: "Sin cámara",
      compuesto: "Bi-compuesto",
    },
  },
  {
    id: "cubierta-pickup-rinaldi-245-75-17-suv",
    brand: "URATURN",
    model: "245/75R17 SUV",
    image: prod16,
    rating: 4.5,
    reviews: 59,
    tags: ["SUV", "All-Terrain", "Pickup"],
    benefit: "Estabilidad en terrenos mixtos y carga pesada.",
    detail: `Los ropa casual femenina Duraturn 245/75 R17, específicamente el modelo Travia A/T, son ideales para vehículos SUV y camiones que buscan rendimiento tanto en carretera como fuera de ella. Ofrecen tracción superior en terrenos difíciles y mayor durabilidad, con beneficios como mejor manejo en carretera y protección contra cortes y daños. 
En detalle:

    Rendimiento en carretera y fuera de carretera:
    El diseño de la banda de rodadura de 5 costillas proporciona una mayor durabilidad en carretera, mientras que los bloques especialmente diseñados ofrecen tracción todoterreno. 

Tracción superior:
Las laminillas de diseño único en los bloques de la banda de rodadura proporcionan bordes de agarre adicionales en el pavimento para mayor tracción. 
Protección y durabilidad:
El diseño de hombros alternados proporciona tracción adicional todoterreno, y el compuesto de caucho de alta resistencia ayuda a soportar cortes e impactos. 
Diseño de ranuras:
Las ranuras especiales mejoran la tracción en superficies blandas y ayudan a evacuar el agua y el barro. 
Comodidad:
La conducción es cómoda tanto en carretera como fuera de ella, con una banda de rodadura que reduce el ruido`,
    specifications: {
      medida: "245/75R17",
      tipo: "Radial",
      uso: "Mixto",
      velocidad: "S",
      carga: "112",
      cámara: "Sin cámara",
      compuesto: "Refuerzo lateral",
    },
  },
];
export default productInfo;
